#!/usr/bin/env bash
#
# Usage: ./convert_to_hls.sh input.mp4 [--watermark]
# Splits input.mp4 into 6 HLS segments (1080p/720p/480p) using libx264,
...
if $WATERMARK; then
  ...
  DRAW0=",${COMMON}:text='1080p H264 %{eif\\:floor(t/${seg_time})\\:d}':x=10:y=10"
  DRAW1=",${COMMON}:text='720p  H264 %{eif\\:floor(t/${seg_time})\\:d}':x=10:y=10"
  DRAW2=",${COMMON}:text='480p  H264 %{eif\\:floor(t/${seg_time})\\:d}':x=10:y=10"
fi
...
(
  ...
  ffmpeg -i "../${INPUT}" \
    ...
    -map "[out0]" -map 0:a -c:v:0 libx264 -b:v:0 5000k -maxrate:v:0 5350k -bufsize:v:0 7500k \
    -map "[out1]" -map 0:a -c:v:1 libx264 -b:v:1 2800k -maxrate:v:1 2996k -bufsize:v:1 4200k \
    -map "[out2]" -map 0:a -c:v:2 libx264 -b:v:2 1400k -maxrate:v:2 1498k -bufsize:v:2 2100k \
...
echo "✅ AVC HLS complete in '${OUT_DIR}/'"
